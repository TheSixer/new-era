...
import { getDefaultProps, IActivityPreSaleModuleProps } from './const'
...
const Component: FC<IActivityPreSaleModuleProps> = (props) => {
  const { componentStyle } = props

  const { style } = useComponentStyle(componentStyle)
  const { localData, showCardTitle, activeIndex, setActiveIndex } = useActivityService(props, { showActivityTitle: '预售活动', btnName: '去购买' })
...
Component.displayName = 'ActivityPreSaleModule'

const ActivityPreSaleModule = memo(Component)
export default ActivityPreSaleModule

export const moduleInfo: IModuleInfo = {
  type: BasicModuleSignEnum.PreSale,
  cname: '预售活动',
  icon,
