...
import { ShipInfoParam } from '@wmeimob/backend-api/src/request/data-contracts'
import { EOrderShipMethod } from '@wmeimob-modules/order-data/enums/EOrderShipMethod'
  ...
  return (
    <DrawerForm {...modalProps} initialValues={{ shippingMethod: 0, isSplit: false }} layout="horizontal" onFinish={handleFormFinish}>
      ...
      <ProFormRadio.Group
        ...
        name="shippingMethod"
        options={[
          { label: '商家配送', value: EOrderShipMethod.Store },
          { label: '无需物流', value: EOrderShipMethod.NoShip }
        ]}
        rules={[{ required: true }]}
      />
      ...
      <ProFormDependency name={['shippingMethod']}>
        {({ shippingMethod }) => {
          const disabled = shippingMethod === EOrderShipMethod.NoShip
