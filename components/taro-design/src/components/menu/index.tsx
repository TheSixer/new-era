...
export default class MMMenu extends Component<IMMMenuProps> {
  ...
  private get style() {
    ...
    const scrollHeight = scrollViewHeight === undefined ? scrollViewHeightState : scrollViewHeight
  ...
  private renderScrollView() {
    ...
    return datas.map((data, index) => (
      <ScrollView scrollY key={index + '1'} style={this.style} className={this.getClassName(index)}>
        {data.map((dataValue) => (
          <View
            onClick={() => this.onChange(dataValue.value, index)}
            className={classNames(styles.item, dataValue.value === value[index] ? styles.selected : '')}
            key={dataValue.value}
          >
            {dataValue.text}
          </View>
  ...
  private async calculateScrollViewHeight() {
    const topViewRes = await selectRect('#' + this.id, Taro.getCurrentInstance().page)
