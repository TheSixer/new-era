...
import { IMMGoodsCreatePageProps, TGoodData, IStepTwoFormDTO } from '../const'
 ...
 *
 * @export
 * @param {IMMGoodsCreatePageProps} props
 * @return {*}
 */
export function useGoodsCreateService(props: IMMGoodsCreatePageProps) {
  const { history } = props
  const { goodsNo, isCopy } = parseHisotry(props.history)
    ...
    async function initData() {
      ...
      if (goodsNo) {
        ...
        const goodsSkuList = goodsSkuDetailList.map((skuDetail, index) => {
          ...
          return {
            ...omit(skuDetail, ['goodsId', 'id', 'uniform']),
 ...
 * @returns
 */
function parseHisotry({ location }) {
  const goodsNo = (location.query?.goodsNo || '') as string
  const isCopy = location.query?.type === 'copy' // 是否是复制
