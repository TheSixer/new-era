...
const { config, goodConfig } = getGlobalData('systemConfig')
  ...
  const columns = useMemo(() => {
    ...
    const commonColumns = [
        ...
        render: (val, _, index) => {
          return (
            <ProFormDigit
              ...
              fieldProps={{ precision: 2, min: 0.01, max: 999999 }}
              rules={[{ required: true }]}
            />
          )
        }
      ...
      config.enableScore && {
        ...
        width: 140,
        render: (_, __, index) => (
          <ProFormDigit name={['goodsSkuList', getIndex(index), 'score']} fieldProps={{ min: 0, max: 999999, precision: 0 }} rules={[{ required: true }]} />
        )
      },
      {
        dataIndex: 'enabled',
