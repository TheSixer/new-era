...
export default function useExport(url = '') {
   ...
   * 导出表格
   * @param params
   * @param exportUrl 指定url
   * @returns
   */
  async function exportTable(params: Record<string, any> = {}, exportUrl = '') {
    if (!url && !!exportUrl) {
      return
    }
    ...
    try {
      ...
      const fetchUrl = jointQuery(exportUrl || url, { ...params, pageNum: 1, pageSize: undefined })
