...
const Component: FC<any> = ({ history }) => {
  ...
  const { request, actionRef } = useProTableRequest((params) => api['/admin/mall/banner/queryList_GET']({...params, position: 'OPEN_SCREEN'}), {
    dataFormat: (data) =>
      data.map((item) => {
        return item
      })
  ...
  const modal = (
    <ModalForm<BannerCreateInputDto>
      ...
      labelCol={{ span: 6 }}
      wrapperCol={{ span: 16 }}
      initialValues={{ url: { type: EJumpType.None, content: {} }, showStatus: 1, sort: 0 }}
