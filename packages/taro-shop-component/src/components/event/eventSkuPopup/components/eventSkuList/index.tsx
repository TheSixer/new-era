...
interface SkuItem { unifyDate?: string, unifyTime?: any[] }
        ...
        <View className={styles.event_ku_list}>
            ...
            <View className={styles.event_ku_list_container}>
                {transformedSchedule.map((item, index) => (
                    <View key={index} className={classNames(styles.event_ku_item, { [styles.active_item]: selectedDate?.unifyDate === item.unifyDate })} onClick={() => handleDateSelect(item)}>
                        <View className={classNames(styles.event_ku_date, { [styles.active_text]: selectedDate?.unifyDate === item.unifyDate })}>{transformDate(item.unifyDate)}</View>
                    </View>
                ))}
            </View>
            {selectedDate?.unifyTime &&
                <>
                    <View className={styles.event_ku_title} style={{ marginTop: '32rpx' }}>选择时段</View>
                    <View className={styles.event_ku_list_container}>
                        {selectedDate?.unifyTime?.map((time, index) => (
                            <View key={time + '_' + index} className={classNames(styles.event_ku_item, { [styles.active_item]: selectedTime === time })} onClick={() => handleTimeSelect(time)}>
